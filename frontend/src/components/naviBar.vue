        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" :class="{ hiddencollapse: isHidden }" id="navbarNav"
                ...
                <ul class="navbar-nav mx-auto">
                    <li class="nav-item">
                        <a class="nav-link" @click.prevent="handleEventsClick" aria-current="page">events</a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link" @click.prevent="handleAttractionsClick" aria-current="page">attractions</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" @click.prevent="handleItineraryClick" aria-current="page">itinerary</a>
                    </li>
                    <li class="nav-item">
                ...
                <ul class="navbar-nav">
                    <li id="last-item" class="nav-item" v-if="!isAuthenticated">
                        <button @click="$emit('toggle-modal')" class="nav-link btn btn-link mt-3 mb-3">login / sign
                            up</button>
                    </li>
    ...
    methods: {
        ...
        handleEventsClick() {
            if (this.isAuthenticated) {
                this.$router.push('/events'); // Navigate to Events Page
            } else {
                this.openLoginPopup(); // Show the login modal
            }
        },

        handleAttractionsClick() {
            if (this.isAuthenticated) {
                this.$router.push('/attractionsList'); // Navigate to Attractions Page
            } else {
                this.openLoginPopup(); // Show the login modal
            }
        },
        ...
        handleItineraryClick() {
            if (this.isAuthenticated) {
                this.$router.push('/itineraryList'); // Navigate to Itinerary Page
            } else {
                this.openLoginPopup(); // Show the login modal
            }
        },
        ...
        openLoginPopup() {
            this.$emit('toggle-modal'); // Emit an event to show the login modal
        },
